    ...
    <aside className="jn-aside">
      <section className="jn-aside-block">
        ...
        {sortedAgenda.length === 0 ? (
          <p className="jn-muted">這天還沒有行程。可在左側時間軸拖曳新增。</p>
        ) : (
          <ul className="jn-agenda-list">
            {sortedAgenda.map((ev) => {
              const isToday = dateKey === dateKeyFromDate(new Date());
              const upcoming =
                isToday && !ev.allDay && ev.startMin > nowMin && ev.startMin - nowMin <= 60;
              const live =
              ...
              return (
                ...
                >
                  <div className="jn-agenda-meta">
                    ...
                    {upcoming && !live && <span className="jn-agenda-badge">即將</span>}
                  </div>
