...
export default function GraphPage() {
  ...
  useEffect(() => {
    if (!user) return;
    positionsRef.current = loadPositions(user.uid);
  }, [user]);
  ...
  useEffect(() => {
    ...
    const writeAll = () => {
      ...
      if (user) savePositions(user.uid, positionsRef.current);
    };
  ...
  }, [full, layout, relayoutNonce, notes.length]);
  ...
  const persistPos = useCallback(() => {
    if (!user) return;
    savePositions(user.uid, positionsRef.current);
  }, [user]);
  ...
  const onPointerUp = () => {
    if (dragRef.current?.kind === "node") persistPos();
